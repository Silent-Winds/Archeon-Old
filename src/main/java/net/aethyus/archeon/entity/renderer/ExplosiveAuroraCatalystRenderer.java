...
@OnlyIn(Dist.CLIENT)
public class ExplosiveAuroraCatalystRenderer {
	public static class ModelRegisterHandler {
		@SubscribeEvent
		@OnlyIn(Dist.CLIENT)
		public void registerModels(ModelRegistryEvent event) {
			RenderingRegistry.registerEntityRenderingHandler(ExplosiveAuroraCatalystEntity.entity, renderManager -> {
				return new MobRenderer(renderManager, new Modelcatalyst_explosive(), 0.5f) {
					{
					...
					public ResourceLocation getEntityTexture(Entity entity) {
						return new ResourceLocation("archeon:textures/entities/explosive_catalyst.png");
					}
				};
			});
		}
	...
	private static class GlowingLayer<T extends Entity, M extends EntityModel<T>> extends LayerRenderer<T, M> {
		...
		public void render(MatrixStack matrixStackIn, IRenderTypeBuffer bufferIn, int packedLightIn, T entitylivingbaseIn, float limbSwing,
			...
			IVertexBuilder ivertexbuilder = bufferIn
					.getBuffer(RenderType.getEyes(new ResourceLocation("archeon:textures/entities/explosive_catalyst.png")));
